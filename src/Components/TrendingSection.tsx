          ...
          <div key={tweet.id} className=" p-1 2xl:p-3  rounded-lg flex items-start space-x-2 md:space-x-3">
            ...
            <div className="flex-1">
              <Link to={`/user/${tweet.user.username}`}> <div className="flex items-center space-x-1 2xl:space-x-2">
                <span  className="text-sm  md:text-base font-semibold whitespace-nowrap overflow-hidden text-ellipsis">{tweet.user.name}</span>
                <span className="text-gray-400   text-sm text-wrap ">@{tweet.user.username}</span>
                {tweet.user.isVerified && <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#a200e8" className="size-6">
