          ...
          <div className="grid grid-cols-18 pb-2 md:pb-10  w-[95%]  mx-auto ">
            <div className="relative flex flex-col  items-start  col-span-18    md:col-span-18    -mt-12">
              ...
              <div className="flex items-center gap-x-2  w-full  ">
                ...
                <Link to={`/level-explain`} className="flex items-center mt-2 relative group">
                  <div className={`px-2 py-1 rounded-full text-xs font-semibold ${leveldata?.data?.toString() === 'LEVEL_0' ? 'bg-gray-500 text-white' :
                    leveldata?.data?.toString() === 'LEVEL_1' ? 'bg-green-500 text-white hover:bg-green-600 cursor-pointer' :
                      leveldata?.data?.toString() === 'LEVEL_2' ? 'bg-blue-500 text-white hover:bg-blue-600 cursor-pointer' :
                        leveldata?.data?.toString() === 'LEVEL_3' ? 'bg-purple-500 text-white hover:bg-purple-600 cursor-pointer' :
                          leveldata?.data?.toString() === 'LEVEL_4' ? 'bg-yellow-500 text-black hover:bg-yellow-600 cursor-pointer' :
                            leveldata?.data?.toString() === 'LEVEL_5' ? 'bg-orange-500 text-white hover:bg-orange-600 cursor-pointer' :
