...
import { getUserFollowerbyuserid, getUserFollowinguserid } from "../utils/apicalls";
      ...
      <div className="w-full  min-h-[96vh] border border-white/10   overflow-y-scroll  max-h-[96vh]  my-[2vh]  mx-auto bg-primaryColor md:bg-secondaryColor text-white rounded-xl overflow-hidden  ">
        ...
        <div className="">
          <div className="relative">
            ...
            <div className="absolute top-4 right-4 flex space-x-2">
              ...
              <button onClick={handleLogout} className="bg-white text-black px-4 py-1 rounded-full border border-black">
                Logout
              </button>
            </div>
          </div>
        ...
        <div className="grid grid-cols-18 pb-2 md:pb-10 border-b border-white/40 w-[95%]  mx-auto ">
          <div className="relative flex flex-col  items-start  col-span-15    md:col-span-7    -mt-12">
            ...
            />
            <h2 className=" text-base  md:text-xl font-semibold mt-2">{user.name}</h2>
            <p className="text-white/60 text-sm mt-2 ">
              @{user.username}  <span className="text-white/30">   • Joined {formatDateTime(user.createdAt)}  </span>
            </p>
          </div>
  
          <div className=" col-span-18  md:col-span-11 py-8   md:py-0    flex justify-between md:justify-around items-center mx-2">
            <div className="text-center">
            <Link to={`/following/${user.id}`}> <p className="text-white/60 text-sm mb-2">Following</p></Link> 
                {userfollowingloading? <SmallLoader/> : 
              <p className="text-lg font-semibold ">{userfollower?.data?.length}</p>}
            </div>
            <div className="text-center">
           <Link to={`/follower/${user.id}`}> <p className="text-white/60 text-sm mb-2">Followers</p></Link> 
            {userfollowerloading? <SmallLoader/> : 
              <p className="text-lg font-semibold ">{userfollowing?.data?.length}</p>}
            </div>
            <div className="text-center">
