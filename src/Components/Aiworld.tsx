...
const Aiworld = () => {
  ...
  const handleCreateUser = async()=>{
    if(twitteruser){
        setuserLoading(true)
        const {data} = await axios.post<CreateUserResponse>(`${import.meta.env.VITE_PUBLIC_BACKEND_URL}/api/user/PremiumUser` , {
  ...
  return (
      ...
      <div className="mt-8">
        <div className="w-[90%]  mx-auto">
          <input
            ...
            className="w-full  mx-auto px-4 py-2 bg-gray-800 text-white placeholder-gray-400  focus:outline-none"
          />
        </div>
    ...
    {twitteruser && 
       ...
       <> 
        {!isLoading &&  <> 
            ...
            {twitteruser.status === "notfound"  && <div className="text-center font-bold mt-5 text-xl">  User not found ! </div>}
    ...
    <div className="mx-6 my-4"> 
      ...
      <div className="w-10 h-10 rounded-full overflow-hidden bg-gray-700 mr-4">
        <img
          src={ formatTwitterAvatarUrl( twitteruser.avatar)} // Replace with your image URL
          alt="User Avatar"
      ...
      <div className="flex-grow">
        <div className="font-bold flex gap-x-2 items-center">  {twitteruser.name}     <span >
        ...
        <div className="text-gray-400 text-sm">  @{twitteruser.profile}  </div>
      ...
      {/* Follow Button */}
      <button onClick={handleCreateUser} className="  bg-gray-200 text-black px-4 py-1  font-bold text-sm hover:bg-gray-300">
        Import
      </button>
    </div>
    ...
    {isLoading && <div className="mx-auto w-full ">  <Loader/> </div>   }
    {userLoading && <div className="flex items-center justify-center flex-col"> 
